...
## Gerar Molde — teto de seleção, chunking e timeout do Tiny

### Teto de 1000 + chunking de `.in()` (2026-06-09)
- `app/(dashboard)/gerar-molde/page.tsx` pede `per_page=1000` e `app/api/pedidos/route.ts` permite até 1000 (era 200). Sem isso a tela só mostrava os 200 mais recentes.
- `app/api/producao/gerar/route.ts` quebra **todos** os filtros `.in(...)` em lotes de 150 ids (`selectInChunks`/`chunkIds`, topo do arquivo). Supabase serializa `.in()` na URL; com ~200+ UUIDs a URL estoura o limite do gateway (~8KB) e a query volta **vazia sem erro** → antes dava 400 "Nenhum pedido valido encontrado" com lote grande.

### Timeout do Tiny síncrono (parede conhecida, NÃO resolvida)
`/api/producao/gerar` processa o Tiny **síncrono** (createExpedition → fetchExpedition → completeExpedition) por grupo, dentro do request. Lote grande (177 pedidos em 09/06) cria ~6-7 expedições (~3 min) e o **proxy mata por timeout** antes de terminar, deixando estado parcial:
- Pedidos `em_producao` sem lote (órfãos) — o claim marca todos no começo.
- Lotes `erro_parcial`/`processando` travado → conferência faltando (a geração de arquivo assíncrona morre junto).
- **N expedições no Tiny vs N-1 no app**: a última é criada no Tiny mas a rota morre antes de gravar lote+expedicao → órfã no Tiny.

**Fix definitivo pendente:** mover o processamento Tiny pra background (responder 202 na hora). Enquanto não fizer, gerar lotes grandes repete o estrago.

**Playbook de recuperação (validado 09/06):**
1. Conferência faltando (lote `erro_parcial`/`processando`): botão **Reprocessar** na Produção (só regenera arquivo, não toca Tiny). Clique ÚNICO — clique duplo causa race "The resource already exists" no upload da conferência.
2. Órfãos (`em_producao` sem lote): reverter pra `pronto_producao` (`UPDATE pedidos`) filtrando por data do claim (`updated_at`) e ausência de `itens_producao.lote_id`, e regerar em lote pequeno (≤100). O auto-skip-já-expedida cobre NFs que a expedição órfã do Tiny já expediu.
3. Expedição órfã no Tiny (sem conferência/chapa): "adotar" no app — cria lote + expedicao apontando pro `tiny_agrupamento_id` existente + roda `processUniqueBoxBatch`. Template: `scripts/adopt-expedicao-5020.ts` (NÃO toca no Tiny). Etiquetas saem do painel do Tiny.

## Intake de pedidos (webhooks Tiny) — INCIDENTE ABERTO (2026-06-09)

### Fluxo do pipeline
```
webhook tiny-pedido (situacao=3 Aprovada, ecommerce Shopify uniquebox/kids)
  → pedidos.status='recebido' + enfileira job fiscal_duplication
  → worker (lib/worker.ts, kickWorker fire-and-forget) roda fiscal_duplication
  → status 'aguardando_nf'
  → webhook nf-autorizada enfileira job enrichment
  → enrichment → status 'pronto_producao' (aparece no Gerar Molde)
```
...
- Uso: `npx tsx scripts/backfill-tiny-pedidos.ts <dataIni> <dataFim> [dry|run] [maxPost]` com `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` do projeto `tkfpbcyjmaifuvfjqobn`.
- **LIMITAÇÃO CRÍTICA:** só ingere situacao=3 (Aprovada). Pedidos que avançaram pra Enviada(5)/Faturada(1) enquanto o webhook estava fora são ignorados. E os ingeridos **travam em `aguardando_nf`** enquanto o webhook `nf-autorizada` estiver fora (o enrichment que move pra `pronto_producao` é disparado por esse webhook). **Ou seja: o backfill completo só fecha DEPOIS de restaurar o webhook.**
- Em 09/06: dos 102 faltantes (#53057–53219), 99 estavam Aprovada; 3 de teste (#53059–61) foram ingeridos e ficaram em `aguardando_nf`. O grosso NÃO foi backfillado de propósito — iam empacar todos em aguardando_nf.

`scripts/probe-tiny-pedidos-hoje.ts` é a versão read-only só pra listar/cruzar.
